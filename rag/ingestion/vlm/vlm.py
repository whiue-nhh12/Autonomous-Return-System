...
def _vlm_api_key(configured_key: str) -> str:
    return (configured_key or os.getenv("VLM_API_KEY") or os.getenv("OPENAI_API_KEY") or "").strip()
...
class VlmEnricher(BaseIngestion):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if model is not None:
            self.settings.vlm_model = model
        self._api_key = api_key

    @property
    def adapter(self) -> VlmAdapter:
        if self._adapter is None:
            api_key = _vlm_api_key(self._api_key or self.settings.vlm_api_key)
            if not api_key:
                raise VlmError(
            ...
            self._adapter = VlmAdapterFactory.create(
                ...
                api_key=api_key,
                base_url=self.settings.vlm_base_url or os.getenv("VLM_BASE_URL"),
            )
    ...
    def run(self, parsed: ParsedDocument) -> ParsedDocument:
        ...
        if self._adapter is None and not _vlm_api_key(self._api_key or self.settings.vlm_api_key):
            self.fail(
            "Thiếu VLM_API_KEY (hoặc OPENAI_API_KEY) trong .env.",
                self.timed_metrics(
                    started,
    ...
    def _infer_block(self, block: ContentBlock) -> str:
        ...
        text = self.adapter.infer(
            ...
            prompt=self.settings.vlm_prompt,
            max_tokens=self.settings.vlm_max_tokens,
            temperature=self.settings.vlm_temperature,
        )
